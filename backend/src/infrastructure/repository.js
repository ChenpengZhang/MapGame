...
export class Repository {
  ...
  async one(sql, args = []) {
    return (await this.db.query(sql, args)).rows[0];
  }
  ...
  // FOR UPDATE 行锁：同一用户的并发开局在这里串行化，配合唯一索引兜底。
  async lockUser(userId) {
    await this.one('SELECT id FROM "user" WHERE id=$1 FOR UPDATE', [userId]);
  }

  async now() {
    return (await this.one('SELECT clock_timestamp() AS now')).now;
  }

  daily(date) {
    return this.one('SELECT * FROM daily_challenges WHERE challenge_date=$1', [date]);
  }

  async createDaily(date, puzzle) {
    await this.db.query(
      `INSERT INTO daily_challenges(id,challenge_date,puzzle,opens_at,closes_at)
  ...
  async active(userId, mode, city, scenario, dailyId) {
    return this.one(
      ...
      AND g.city_id=$3 AND g.scenario_key=$4 AND g.daily_challenge_id IS NOT DISTINCT FROM $5::uuid`,
      [userId, mode, city, scenario, dailyId],
    );
  }

  async createRun(userId, mode, puzzle, dailyId) {
    const run = await this.one(
      `INSERT INTO game_runs(id,user_id,mode,daily_challenge_id,city_id,scenario_key,data_hash,rules_version)
  ...
  stage(id) {
    return this.one('SELECT *,round_no AS stage_no FROM run_rounds WHERE id=$1', [id]);
  }

  latestStage(runId) {
    return this.one(
      'SELECT *,round_no AS stage_no FROM run_rounds WHERE run_id=$1 ORDER BY round_no DESC LIMIT 1',
      [runId],
    );
  }
  ...
  createStage(runId, number, puzzle) {
    return this.one(
      `INSERT INTO run_rounds(id,run_id,round_no,puzzle,optimal_duration_ms)
      VALUES($1,$2,$3,$4,$5) RETURNING *,round_no AS stage_no`,
      [randomUUID(), runId, number, puzzle, puzzle.optimalDurationMs],
    );
  }

  submission(requestId) {
    return this.one('SELECT *,round_id AS stage_id FROM round_submissions WHERE request_id=$1', [requestId]);
  }

  dailyById(id) {
    return this.one('SELECT * FROM daily_challenges WHERE id=$1', [id]);
  }

  // 结算是一个多步状态机：写提交 → 更新回合状态 → 更新对局状态 →
  ...
  async record(run, stage, requestId, route, result, receivedAt) {
    ...
    await this.db.query(
      'UPDATE run_rounds SET status=$2 WHERE id=$1',
      [stage.id, result.passed ? 'passed' : 'failed'],
    );
    ...
    if (run.mode === 'tower' && result.passed) {
      ...
      await this.db.query(
        `INSERT INTO timed_run_results(run_id,duration_ms,achieved_at)
  ...
  async abandon(id) {
    await this.db.query("UPDATE game_runs SET status='abandoned',finished_at=clock_timestamp() WHERE id=$1", [id]);
  }

  async abandonPractice(userId) {
    await this.db.query(
      "UPDATE game_runs SET status='abandoned',finished_at=clock_timestamp() WHERE user_id=$1 AND mode IN ('free','story') AND status='active'",
      [userId],
    );
  }
  ...
  async towerProgress(userId, city) {
    return (
      ...
      WHERE g.user_id=$1 AND g.city_id=$2 AND g.mode='tower' GROUP BY g.scenario_key`,
        [userId, city],
      )
    ).rows;
  }
  ...
  async saves(userId) {
    return (
      await this.db.query(
        `SELECT g.id,g.mode,g.city_id,g.scenario_key,t.cleared_layers,g.created_at
      FROM game_runs g JOIN tower_run_stats t ON t.run_id=g.id
      WHERE g.user_id=$1 AND g.mode='tower' AND g.status='active' ORDER BY g.created_at DESC`,
        [userId],
      )
    ).rows;
  }
