...
export async function ensureDatabase(config) {
  const target = new pg.Client({ connectionString: config.DATABASE_URL, connectionTimeoutMillis: 5000 });
  ...
  try {
    ...
    if (error.code !== '3D000') throw new Error(`Cannot connect to PostgreSQL (${error.code ?? 'connection error'}); check DATABASE_URL and server availability`);
  ...
  const admin = config.DATABASE_ADMIN_URL
    ...
    : new pg.Client({ host: params.host, port: params.port, user: params.user, password: params.password,
      ssl: params.ssl, database: 'postgres', connectionTimeoutMillis: 5000 });
  if (admin.connectionParameters.host !== params.host || admin.connectionParameters.port !== params.port) {
  ...
  try {
    ...
    const exists = async () => (await admin.query('SELECT 1 FROM pg_database WHERE datname=$1',[params.database])).rowCount > 0;
    ...
    if (error.code === '42501') throw new Error('Database creation requires CREATEDB and permission to assign the DATABASE_URL role as owner; configure DATABASE_ADMIN_URL or grant those permissions');
    throw new Error(`Cannot create PostgreSQL database (${error.code ?? 'connection error'}); check DATABASE_ADMIN_URL and the existing login role`);
  } finally {
...
export async function bootstrapDatabase(config,sendMail) {
  ...
  const pool = new pg.Pool({ connectionString: config.DATABASE_URL,max: 10,connectionTimeoutMillis: 5000 });
  ...
  try {
    ...
    await pool.end();
    throw new Error(`Database migration failed (${error.code ?? 'schema or migration mismatch'}); API startup stopped. Check database DDL permissions and migration files`);
  }
}
