...
export function authOptions(config, pool, sendMail) {
  return {
    appName: 'MapGame', database: pool,
    baseURL: config.PUBLIC_ORIGIN, basePath: '/mapgame/api/auth',
    secret: config.BETTER_AUTH_SECRET, trustedOrigins: [config.PUBLIC_ORIGIN],
    emailAndPassword: {
    ...
    emailVerification: {
      sendOnSignUp: true, sendOnSignIn: true, expiresIn: 600,
    },
    plugins: [emailOTP({
    ...
    advanced: {
      cookiePrefix: 'mapgame', useSecureCookies: config.NODE_ENV === 'production',
