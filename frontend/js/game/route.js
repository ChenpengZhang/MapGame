...
export function onStopClick(e) {
  ...
  if (state.isTouch) {
    ...
    startRoute({ logical, point: phys.lnglat,physicalStopId:String(phys.id) });
  }
}
...
export function onCandidateStopClick(phys) {
  ...
  if (state.isTouch) {
    ...
    if (state.showAllStops) { showCenterToast('请关闭全图显示后继续'); return; }
    commitCandidate(logical, prev,phys);
  }
}
...
/** 手机端选下一站 · 第二步：确定 */
function confirmCandidate(logical,phys) {
  state.pendingCandidate = null;
  commitCandidate(logical, state.routeStops[state.routeStops.length - 1],phys);
}
