 ...
 *
 * 【分层说明】
 *   本文件属于 core（基础层）。core 只放常量和底层配置，
 *   不依赖 data / map / game / ui。寻路相关数值来自 shared/router.js，
 *   防止玩家计时与系统最优使用两套参数。
 *
 * 【想调数值就改这里】
 ...
 *
 * 调整速度、停站、换乘、等车、合并距离或步行上限时，只改
 * shared/router.js 的 DEFAULT_PARAMS；本文件负责给前端提供兼容命名。
 */

import { DEFAULT_PARAMS as ROUTER_DEFAULTS } from './router-api.js';
...
export const METRO_SPEED_KMH = ROUTER_DEFAULTS.metroSpeedKmh;
export const METRO_DWELL_MIN = ROUTER_DEFAULTS.metroDwellMin;
export const BUS_DWELL_MIN = ROUTER_DEFAULTS.busDwellMin;

// 公交车段行驶模型：0 → 缓慢加速 → 最高巡航速度 → 匀速（路程 = ∫v dt）
// 公交巡航速度按线路平均站间距动态区分：城区密站慢车 → 郊区大站/快车
export const BUS_VMAX_KMH = ROUTER_DEFAULTS.busVmaxKmh;
export const BUS_VMAX_LOCAL_KMH = ROUTER_DEFAULTS.busVmaxLocalKmh;
export const BUS_SPACING_LOCAL_KM = ROUTER_DEFAULTS.busSpacingLocalKm;
export const BUS_SPACING_EXPRESS_KM = ROUTER_DEFAULTS.busSpacingExpressKm;
export const BUS_ACCEL_MPS2 = ROUTER_DEFAULTS.busAccelMps2;

// 换乘惩罚（按前后线路模式区分）
export const BUS_BUS_TRANSFER_MIN = ROUTER_DEFAULTS.busBusTransferMin;
export const METRO_METRO_TRANSFER_MIN = ROUTER_DEFAULTS.metroMetroTransferMin;
export const BUS_METRO_TRANSFER_MIN = ROUTER_DEFAULTS.busMetroTransferMin;

// 等车时间（发车间隔/2 的粗略估计，无时刻表数据，后续可校准）
export const METRO_WAIT_MIN = ROUTER_DEFAULTS.metroWaitMin;
export const BUS_WAIT_MIN = ROUTER_DEFAULTS.busWaitMin;

// ============ 站点合并与步行规则 ============

/** 同名站点合并距离（公交↔地铁换乘，曾为 500 太激进） */
export const MERGE_DISTANCE_M = ROUTER_DEFAULTS.mergeDistanceM;

/** 起终点步行上限：不允许超过 1.5km（超过则禁止该站点作为起终点） */
export const MAX_WALK_M = ROUTER_DEFAULTS.maxWalkKm * 1000;
